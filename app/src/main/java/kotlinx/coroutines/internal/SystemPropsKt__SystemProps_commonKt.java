...
public final /* synthetic */ class SystemPropsKt__SystemProps_commonKt {
    ...
    public static final long systemProp(String str, long j, long j2, long j3) {
        ...
        if (longOrNull == null) {
            throw new IllegalStateException(("System property '" + str + "' has unrecognized value '" + strSystemProp + Operators.SINGLE_QUOTE).toString());
        }
        long jLongValue = longOrNull.longValue();
        if (j2 <= jLongValue && jLongValue <= j3) {
            return jLongValue;
        }
        throw new IllegalStateException(("System property '" + str + "' should be in range " + j2 + PdrUtil.FILE_PATH_ENTRY_BACK + j3 + ", but is '" + jLongValue + Operators.SINGLE_QUOTE).toString());
    }
}
