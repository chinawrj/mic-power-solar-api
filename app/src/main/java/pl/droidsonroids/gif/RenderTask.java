...
class RenderTask extends SafeRunnable {
    ...
    public void doWork() {
        long jRenderFrame = this.mGifDrawable.mNativeInfoHandle.renderFrame(this.mGifDrawable.mBuffer);
        if (jRenderFrame >= 0) {
            ...
            if (this.mGifDrawable.isVisible() && this.mGifDrawable.mIsRunning && !this.mGifDrawable.mIsRenderingTriggeredOnDraw) {
                ...
                this.mGifDrawable.mRenderTaskSchedule = this.mGifDrawable.mExecutor.schedule(this, jRenderFrame, TimeUnit.MILLISECONDS);
            }
