...
public final class Response implements Closeable {
    ...
    public ResponseBody peekBody(long j) throws IOException {
        ...
        Buffer bufferM244clone = bufferedSourceSource.buffer().m244clone();
        if (bufferM244clone.size() > j) {
            Buffer buffer = new Buffer();
            buffer.write(bufferM244clone, j);
            bufferM244clone.clear();
            bufferM244clone = buffer;
        }
        return ResponseBody.create(this.body.contentType(), bufferM244clone.size(), bufferM244clone);
    }
