...
final class ContinuationOutcomeReceiver<R, E extends Throwable> extends AtomicBoolean implements OutcomeReceiver<R, E> {
    ...
    public void onResult(R result) {
        if (compareAndSet(false, true)) {
            ...
            continuation.resumeWith(Result.m326constructorimpl(result));
        }
    }
    ...
    public void onError(E error) {
        if (compareAndSet(false, true)) {
            Continuation<R> continuation = this.continuation;
            Result.Companion companion = Result.INSTANCE;
            continuation.resumeWith(Result.m326constructorimpl(ResultKt.createFailure(error)));
        }
    }

    @Override // java.util.concurrent.atomic.AtomicBoolean
    public String toString() {
        return "ContinuationOutcomeReceiver(outcomeReceived = " + get() + Operators.BRACKET_END;
    }
}
