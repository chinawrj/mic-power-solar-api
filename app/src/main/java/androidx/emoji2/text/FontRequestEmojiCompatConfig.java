        ...
        private FontsContractCompat.FontInfo retrieveFontInfo() {
            try {
                ...
                if (fontFamilyResultFetchFonts.getStatusCode() != 0) {
                    throw new RuntimeException("fetchFonts failed (" + fontFamilyResultFetchFonts.getStatusCode() + Operators.BRACKET_END_STR);
                }
        ...
        void createMetadata() {
            synchronized (this.mLock) {
                ...
                try {
                    ...
                    if (resultCode != 0) {
                        throw new RuntimeException("fetchFonts result is not OK. (" + resultCode + Operators.BRACKET_END_STR);
                    }
                    try {
