...
public class StagingArea {
    ...
    public boolean remove(final CacheKey key) {
        ...
        Preconditions.checkNotNull(key);
        synchronized (this) {
            encodedImageRemove = this.mMap.remove(key);
        }
    ...
    public synchronized EncodedImage get(final CacheKey key) {
        Preconditions.checkNotNull(key);
        EncodedImage encodedImageCloneOrNull = this.mMap.get(key);
        if (encodedImageCloneOrNull != null) {
            synchronized (encodedImageCloneOrNull) {
                if (!EncodedImage.isValid(encodedImageCloneOrNull)) {
                    this.mMap.remove(key);
                    FLog.w(TAG, "Found closed reference %d for key %s (%d)", Integer.valueOf(System.identityHashCode(encodedImageCloneOrNull)), key.getUriString(), Integer.valueOf(System.identityHashCode(key)));
                    return null;
                }
