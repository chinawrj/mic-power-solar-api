...
public abstract class AbstractAdaptiveCountingMemoryCache<K, V> implements CountingMemoryCache<K, V> {
    ...
    public CloseableReference<V> cache(final K key, final CloseableReference<V> valueRef, @Nullable final CountingMemoryCache.EntryStateObserver<K> observer) {
        ...
        synchronized (this) {
            entryRemove = this.mLeastFrequentlyUsedExclusiveEntries.remove(key);
            entryRemove2 = this.mMostFrequentlyUsedExclusiveEntries.remove(key);
            Preconditions.checkState(entryRemove == null || entryRemove2 == null);
            CountingMemoryCache.Entry<K, V> entryRemove3 = this.mCachedEntries.remove(key);
            ...
            if (canCacheNewValue(valueRef.get())) {
                CountingMemoryCache.Entry<K, V> entryOf = CountingMemoryCache.Entry.of(key, valueRef, observer);
                Integer value = this.mLeastFrequentlyUsedKeysGhostList.getValue(key);
                entryOf.accessCount = value != null ? value.intValue() : 0;
                this.mCachedEntries.put(key, entryOf);
    ...
    */
    ...
    public void probe(K key) {
        Preconditions.checkNotNull(key);
        synchronized (this) {
            CountingMemoryCache.Entry<K, V> entryRemove = this.mLeastFrequentlyUsedExclusiveEntries.remove(key);
            if (entryRemove == null) {
                entryRemove = this.mMostFrequentlyUsedExclusiveEntries.remove(key);
            }
            if (entryRemove != null) {
    ...
    public CloseableReference<V> reuse(K key) {
        ...
        synchronized (this) {
            ...
            if (entryRemove != null) {
                CountingMemoryCache.Entry<K, V> entryRemove2 = this.mCachedEntries.remove(key);
    ...
    private synchronized ArrayList<CountingMemoryCache.Entry<K, V>> trimExclusivelyOwnedEntries(int i, int i2, CountingLruMap<K, CountingMemoryCache.Entry<K, V>> countingLruMap, ArrayListType arrayListType) {
        ...
        while (true) {
            ...
            addElementToGhostList(objCheckNotNull, ((CountingMemoryCache.Entry) Preconditions.checkNotNull(countingLruMap.get(objCheckNotNull))).accessCount, arrayListType);
            countingLruMap.remove(objCheckNotNull);
            immutableList.add(this.mCachedEntries.remove(objCheckNotNull));
        }
    }
    ...
    class IntMapArrayList<E> {
        ...
        public void increaseValueIfExists(E element) {
            int iIndexOf = this.mFirstList.indexOf(element);
            if (iIndexOf < 0) {
                return;
            }
