...
public class ThumbnailBranchProducer implements Producer<EncodedImage> {
    ...
    public boolean produceResultsFromThumbnailProducer(int startIndex, Consumer<EncodedImage> consumer, ProducerContext context) {
        int iFindFirstProducerForSize = findFirstProducerForSize(startIndex, context.getImageRequest().getResizeOptions());
        if (iFindFirstProducerForSize == -1) {
            return false;
        }
        this.mThumbnailProducers[iFindFirstProducerForSize].produceResults(new ThumbnailConsumer(consumer, context, iFindFirstProducerForSize), context);
        return true;
    }
