...
public class PartialDiskCacheProducer implements Producer<EncodedImage> {
    ...
    private static class PartialDiskCacheConsumer extends DelegatingConsumer<EncodedImage, EncodedImage> {
        ...
        private PooledByteBufferOutputStream merge(EncodedImage initialData, EncodedImage remainingData) throws IOException {
            ...
            PooledByteBufferOutputStream pooledByteBufferOutputStreamNewOutputStream = this.mPooledByteBufferFactory.newOutputStream(remainingData.getSize() + i);
            copy(initialData.getInputStreamOrThrow(), pooledByteBufferOutputStreamNewOutputStream, i);
            copy(remainingData.getInputStreamOrThrow(), pooledByteBufferOutputStreamNewOutputStream, remainingData.getSize());
        ...
        private void copy(InputStream from, OutputStream to, int length) throws IOException {
            ...
            while (i > 0) {
                try {
                    int i2 = from.read(bArr, 0, Math.min(16384, i));
                    if (i2 < 0) {
                        break;
                    } else if (i2 > 0) {
                        to.write(bArr, 0, i2);
        ...
        private void sendFinalResultToConsumer(PooledByteBufferOutputStream pooledOutputStream) throws Throwable {
