            ...
            try {
                ...
                if (!isNotLast(status) && newResult != null && !statusHasAnyFlag(status, 10) && newResult.getImageFormat() != ImageFormat.UNKNOWN) {
                    ...
                    if (byteBufferRef != null) {
                        try {
                            CloseableReference<PooledByteBuffer> closeableReferenceCache = (this.mEncodedCacheEnabled && this.mIsMemoryCacheEnabled) ? this.mMemoryCache.cache(this.mRequestedCacheKey, byteBufferRef) : null;
                            if (closeableReferenceCache != null) {
                                try {
                                    EncodedImage encodedImage = new EncodedImage(closeableReferenceCache);
