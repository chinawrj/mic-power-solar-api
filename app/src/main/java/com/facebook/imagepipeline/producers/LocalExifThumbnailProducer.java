...
public class LocalExifThumbnailProducer implements ThumbnailProducer<EncodedImage> {
    ...
    public EncodedImage buildEncodedImage(PooledByteBuffer imageBytes, ExifInterface exifInterface) {
        Pair<Integer, Integer> pairDecodeDimensions = BitmapUtil.decodeDimensions(new PooledByteBufferInputStream(imageBytes));
        ...
        CloseableReference closeableReferenceOf = CloseableReference.of(imageBytes);
        try {
    ...
    private class Api24Utils {
        ...
        ExifInterface getExifInterface(FileDescriptor fileDescriptor) throws IOException {
            if (Build.VERSION.SDK_INT >= 24) {
                return Chip$$ExternalSyntheticApiModelOutline0.m(fileDescriptor);
            }
            return null;
        }
    }
