...
public class RequestLoggingListener implements RequestListener {
    ...
    public synchronized void onProducerFinishWithSuccess(String requestId, String producerName, @Nullable Map<String, String> extraMap) {
        if (FLog.isLoggable(2)) {
            Long lRemove = this.mProducerStartTimeMap.remove(Pair.create(requestId, producerName));
            long time = getTime();
            FLog.v(TAG, "time %d: onProducerFinishWithSuccess: {requestId: %s, producer: %s, elapsedTime: %d ms, extraMap: %s}", Long.valueOf(time), requestId, producerName, Long.valueOf(getElapsedTime(lRemove, time)), extraMap);
        }
    }
    ...
    public synchronized void onProducerFinishWithFailure(String requestId, String producerName, Throwable throwable, @Nullable Map<String, String> extraMap) {
        if (FLog.isLoggable(5)) {
            Long lRemove = this.mProducerStartTimeMap.remove(Pair.create(requestId, producerName));
            long time = getTime();
            FLog.w(TAG, throwable, "time %d: onProducerFinishWithFailure: {requestId: %s, stage: %s, elapsedTime: %d ms, extraMap: %s, throwable: %s}", Long.valueOf(time), requestId, producerName, Long.valueOf(getElapsedTime(lRemove, time)), extraMap, throwable.toString());
        }
    }
    ...
    public synchronized void onProducerFinishWithCancellation(String requestId, String producerName, @Nullable Map<String, String> extraMap) {
        if (FLog.isLoggable(2)) {
            Long lRemove = this.mProducerStartTimeMap.remove(Pair.create(requestId, producerName));
            long time = getTime();
            FLog.v(TAG, "time %d: onProducerFinishWithCancellation: {requestId: %s, stage: %s, elapsedTime: %d ms, extraMap: %s}", Long.valueOf(time), requestId, producerName, Long.valueOf(getElapsedTime(lRemove, time)), extraMap);
        }
    }
    ...
    public synchronized void onUltimateProducerReached(String requestId, String producerName, boolean successful) {
        if (FLog.isLoggable(2)) {
            Long lRemove = this.mProducerStartTimeMap.remove(Pair.create(requestId, producerName));
            long time = getTime();
            FLog.v(TAG, "time %d: onUltimateProducerReached: {requestId: %s, producer: %s, elapsedTime: %d ms, success: %b}", Long.valueOf(time), requestId, producerName, Long.valueOf(getElapsedTime(lRemove, time)), Boolean.valueOf(successful));
        }
    }
    ...
    public synchronized void onRequestSuccess(ImageRequest request, String requestId, boolean isPrefetch) {
        if (FLog.isLoggable(2)) {
            Long lRemove = this.mRequestStartTimeMap.remove(requestId);
            long time = getTime();
            FLog.v(TAG, "time %d: onRequestSuccess: {requestId: %s, elapsedTime: %d ms}", Long.valueOf(time), requestId, Long.valueOf(getElapsedTime(lRemove, time)));
        }
    }
    ...
    public synchronized void onRequestFailure(ImageRequest request, String requestId, Throwable throwable, boolean isPrefetch) {
        if (FLog.isLoggable(5)) {
            Long lRemove = this.mRequestStartTimeMap.remove(requestId);
            long time = getTime();
            FLog.w(TAG, "time %d: onRequestFailure: {requestId: %s, elapsedTime: %d ms, throwable: %s}", Long.valueOf(time), requestId, Long.valueOf(getElapsedTime(lRemove, time)), throwable.toString());
        }
    }
    ...
    public synchronized void onRequestCancellation(String requestId) {
        if (FLog.isLoggable(2)) {
            Long lRemove = this.mRequestStartTimeMap.remove(requestId);
            long time = getTime();
            FLog.v(TAG, "time %d: onRequestCancellation: {requestId: %s, elapsedTime: %d ms}", Long.valueOf(time), requestId, Long.valueOf(getElapsedTime(lRemove, time)));
        }
    }
