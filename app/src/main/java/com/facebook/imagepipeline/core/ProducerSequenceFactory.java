...
public class ProducerSequenceFactory {
    ...
    private synchronized Producer<EncodedImage> getCommonNetworkFetchToEncodedMemorySequence() {
        Producer<EncodedImage> producerNewEncodedCacheMultiplexToTranscodeSequence;
        if (FrescoSystrace.isTracing()) {
        ...
        if (this.mCommonNetworkFetchToEncodedMemorySequence == null) {
            ...
            if (this.mUseCombinedNetworkAndCacheProducer) {
                producerNewEncodedCacheMultiplexToTranscodeSequence = this.mProducerFactory.newCombinedNetworkAndCacheProducer(this.mNetworkFetcher);
            } else {
                producerNewEncodedCacheMultiplexToTranscodeSequence = newEncodedCacheMultiplexToTranscodeSequence(this.mProducerFactory.newNetworkFetchProducer(this.mNetworkFetcher));
            }
            AddImageTransformMetaDataProducer addImageTransformMetaDataProducerNewAddImageTransformMetaDataProducer = ProducerFactory.newAddImageTransformMetaDataProducer((Producer) Preconditions.checkNotNull(producerNewEncodedCacheMultiplexToTranscodeSequence));
            this.mCommonNetworkFetchToEncodedMemorySequence = addImageTransformMetaDataProducerNewAddImageTransformMetaDataProducer;
            this.mCommonNetworkFetchToEncodedMemorySequence = this.mProducerFactory.newResizeAndRotateProducer(addImageTransformMetaDataProducerNewAddImageTransformMetaDataProducer, this.mResizeAndRotateEnabledForNetwork && !this.mDownsampleEnabled, this.mImageTranscoderFactory);
            if (FrescoSystrace.isTracing()) {
                FrescoSystrace.endSection();
            }
        }
    ...
    private synchronized Producer<CloseableReference<CloseableImage>> getDataFetchSequence() {
        if (this.mDataFetchSequence == null) {
            Producer<EncodedImage> producerNewDataFetchProducer = this.mProducerFactory.newDataFetchProducer();
            if (WebpSupportStatus.sIsWebpSupportRequired && (!this.mWebpSupportEnabled || WebpSupportStatus.sWebpBitmapFactory == null)) {
                producerNewDataFetchProducer = this.mProducerFactory.newWebpTranscodeProducer(producerNewDataFetchProducer);
            }
            this.mDataFetchSequence = newBitmapCacheGetToDecodeSequence(this.mProducerFactory.newResizeAndRotateProducer(ProducerFactory.newAddImageTransformMetaDataProducer(producerNewDataFetchProducer), true, this.mImageTranscoderFactory));
        }
    ...
    private Producer<CloseableReference<CloseableImage>> newBitmapCacheGetToDecodeSequence(Producer<EncodedImage> inputProducer) {
        ...
        Producer<CloseableReference<CloseableImage>> producerNewBitmapCacheGetToBitmapCacheSequence = newBitmapCacheGetToBitmapCacheSequence(this.mProducerFactory.newDecodeProducer(inputProducer));
        if (FrescoSystrace.isTracing()) {
            FrescoSystrace.endSection();
        }
        return producerNewBitmapCacheGetToBitmapCacheSequence;
    }
    ...
    private Producer<EncodedImage> newEncodedCacheMultiplexToTranscodeSequence(Producer<EncodedImage> inputProducer) {
        ...
        EncodedMemoryCacheProducer encodedMemoryCacheProducerNewEncodedMemoryCacheProducer = this.mProducerFactory.newEncodedMemoryCacheProducer(inputProducer);
        if (this.mIsDiskCacheProbingEnabled) {
            return this.mProducerFactory.newEncodedCacheKeyMultiplexProducer(this.mProducerFactory.newEncodedProbeProducer(encodedMemoryCacheProducerNewEncodedMemoryCacheProducer));
        }
        return this.mProducerFactory.newEncodedCacheKeyMultiplexProducer(encodedMemoryCacheProducerNewEncodedMemoryCacheProducer);
    }
    ...
    private Producer<EncodedImage> newDiskCacheSequence(Producer<EncodedImage> inputProducer) {
        ...
        if (this.mPartialImageCachingEnabled) {
            diskCacheWriteProducerNewDiskCacheWriteProducer = this.mProducerFactory.newDiskCacheWriteProducer(this.mProducerFactory.newPartialDiskCacheProducer(inputProducer));
        } else {
            diskCacheWriteProducerNewDiskCacheWriteProducer = this.mProducerFactory.newDiskCacheWriteProducer(inputProducer);
        }
        DiskCacheReadProducer diskCacheReadProducerNewDiskCacheReadProducer = this.mProducerFactory.newDiskCacheReadProducer(diskCacheWriteProducerNewDiskCacheWriteProducer);
        if (FrescoSystrace.isTracing()) {
            FrescoSystrace.endSection();
        }
    ...
    private Producer<CloseableReference<CloseableImage>> newBitmapCacheGetToBitmapCacheSequence(Producer<CloseableReference<CloseableImage>> inputProducer) {
        Producer<CloseableReference<CloseableImage>> producerNewBackgroundThreadHandoffProducer = this.mProducerFactory.newBackgroundThreadHandoffProducer(this.mProducerFactory.newBitmapMemoryCacheKeyMultiplexProducer(this.mProducerFactory.newBitmapMemoryCacheProducer(inputProducer)), this.mThreadHandoffProducerQueue);
        if (this.mIsEncodedMemoryCacheProbingEnabled || this.mIsDiskCacheProbingEnabled) {
            return this.mProducerFactory.newBitmapProbeProducer(this.mProducerFactory.newBitmapMemoryCacheGetProducer(producerNewBackgroundThreadHandoffProducer));
        }
        return this.mProducerFactory.newBitmapMemoryCacheGetProducer(producerNewBackgroundThreadHandoffProducer);
    }
    ...
    private synchronized Producer<CloseableReference<CloseableImage>> getPostprocessorSequence(Producer<CloseableReference<CloseableImage>> inputProducer) {
        ...
        producerNewPostprocessorBitmapMemoryCacheProducer = this.mPostprocessorSequences.get(inputProducer);
        if (producerNewPostprocessorBitmapMemoryCacheProducer == null) {
            producerNewPostprocessorBitmapMemoryCacheProducer = this.mProducerFactory.newPostprocessorBitmapMemoryCacheProducer(this.mProducerFactory.newPostprocessorProducer(inputProducer));
            this.mPostprocessorSequences.put(inputProducer, producerNewPostprocessorBitmapMemoryCacheProducer);
        }
    ...
    private synchronized Producer<Void> getDecodedImagePrefetchSequence(Producer<CloseableReference<CloseableImage>> inputProducer) {
        ...
        producerNewSwallowResultProducer = this.mCloseableImagePrefetchSequences.get(inputProducer);
        if (producerNewSwallowResultProducer == null) {
            producerNewSwallowResultProducer = this.mProducerFactory.newSwallowResultProducer(inputProducer);
            this.mCloseableImagePrefetchSequences.put(inputProducer, producerNewSwallowResultProducer);
        }
    ...
    private synchronized Producer<CloseableReference<CloseableImage>> getBitmapPrepareSequence(Producer<CloseableReference<CloseableImage>> inputProducer) {
        ...
        producerNewBitmapPrepareProducer = this.mBitmapPrepareSequences.get(inputProducer);
        if (producerNewBitmapPrepareProducer == null) {
            producerNewBitmapPrepareProducer = this.mProducerFactory.newBitmapPrepareProducer(inputProducer);
            this.mBitmapPrepareSequences.put(inputProducer, producerNewBitmapPrepareProducer);
        }
