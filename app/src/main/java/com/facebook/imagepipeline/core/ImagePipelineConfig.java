...
public class ImagePipelineConfig implements ImagePipelineConfigInterface {
    ...
    private ImagePipelineConfig(Builder builder) {
        Supplier<MemoryCacheParams> defaultBitmapMemoryCacheParamsSupplier;
        ...
        ImagePipelineExperiments imagePipelineExperimentsBuild = builder.mExperimentsBuilder.build();
        this.mImagePipelineExperiments = imagePipelineExperimentsBuild;
        if (builder.mBitmapMemoryCacheParamsSupplier == null) {
            defaultBitmapMemoryCacheParamsSupplier = new DefaultBitmapMemoryCacheParamsSupplier((ActivityManager) Preconditions.checkNotNull(builder.mContext.getSystemService("activity")));
        } else {
        ...
        if (builder.mCacheKeyFactory == null) {
            ...
            defaultCacheKeyFactory = builder.mCacheKeyFactory;
        }
        ...
        if (builder.mImageCacheStatsTracker == null) {
            noOpImageCacheStatsTracker = NoOpImageCacheStatsTracker.getInstance();
        } else {
            noOpImageCacheStatsTracker = builder.mImageCacheStatsTracker;
        }
        ...
        this.mMemoryChunkType = getMemoryChunkType(builder, imagePipelineExperimentsBuild);
        ...
        WebpBitmapFactory webpBitmapFactory = imagePipelineExperimentsBuild.getWebpBitmapFactory();
        if (webpBitmapFactory != null) {
            setWebpBitmapFactory(webpBitmapFactory, imagePipelineExperimentsBuild, new HoneycombBitmapCreator(getPoolFactory()));
        } else if (imagePipelineExperimentsBuild.isWebpSupportEnabled() && WebpSupportStatus.sIsWebpSupportRequired && (webpBitmapFactoryLoadWebpBitmapFactoryIfExists = WebpSupportStatus.loadWebpBitmapFactoryIfExists()) != null) {
            setWebpBitmapFactory(webpBitmapFactoryLoadWebpBitmapFactoryIfExists, imagePipelineExperimentsBuild, new HoneycombBitmapCreator(getPoolFactory()));
        }
        if (FrescoSystrace.isTracing()) {
            FrescoSystrace.endSection();
        }
