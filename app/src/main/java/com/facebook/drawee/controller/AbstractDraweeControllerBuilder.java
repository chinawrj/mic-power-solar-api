...
public abstract class AbstractDraweeControllerBuilder<BUILDER extends AbstractDraweeControllerBuilder<BUILDER, REQUEST, IMAGE, INFO>, REQUEST, IMAGE, INFO> implements SimpleDraweeControllerBuilder {
    ...
    protected Supplier<DataSource<IMAGE>> obtainDataSourceSupplier(final DraweeController controller, final String controllerId) {
        ...
        if (request != null) {
            firstAvailableDataSourceSupplier = getDataSourceSupplierForRequest(controller, controllerId, request);
        } else {
            REQUEST[] requestArr = this.mMultiImageRequests;
            firstAvailableDataSourceSupplier = requestArr != null ? getFirstAvailableDataSourceSupplier(controller, controllerId, requestArr, this.mTryCacheOnlyFirst) : null;
        }
    ...
    protected Supplier<DataSource<IMAGE>> getDataSourceSupplierForRequest(final DraweeController controller, final String controllerId, final REQUEST imageRequest, final CacheLevel cacheLevel) {
        ...
        return new Supplier<DataSource<IMAGE>>() { // from class: com.facebook.drawee.controller.AbstractDraweeControllerBuilder.2
            ...
            public String toString() {
                return Objects.toStringHelper(this).add(AbsURIAdapter.REQUEST, imageRequest.toString()).toString();
            }
        };
    }
