...
public abstract class AbstractDraweeController<T, INFO> implements DraweeController, DeferredReleaser.Releasable, GestureDetector.ClickListener {
    ...
    private static final Map<String, Object> SHORTCUT_EXTRAS = ImmutableMap.of("origin", "memory_bitmap", ProducerContext.ExtraKeys.ORIGIN_SUBCATEGORY, IApp.ConfigProperty.CONFIG_SHORTCUT);
    ...
    public void onNewResultInternal(String id, DataSource<T> dataSource, @Nullable T image, float progress, boolean isFinished, boolean wasImmediate, boolean deliverTempResult) {
        try {
            ...
            try {
                Drawable drawableCreateDrawable = createDrawable(image);
                T t = this.mFetchedImage;
                Drawable drawable = this.mDrawable;
                ...
                try {
                    if (isFinished) {
                        ...
                        this.mDataSource = null;
                        this.mSettableDraweeHierarchy.setImage(drawableCreateDrawable, 1.0f, wasImmediate);
                        ...
                        this.mSettableDraweeHierarchy.setImage(drawableCreateDrawable, progress, wasImmediate);
    ...
    private void reportFailure(Throwable throwable, @Nullable DataSource<T> dataSource) {
        ControllerListener2.Extras extrasObtainExtras = obtainExtras(dataSource, (DataSource<T>) null, (Uri) null);
        getControllerListener().onFailure(this.mId, throwable);
        getControllerListener2().onFailure(this.mId, throwable, extrasObtainExtras);
    }
    ...
    private ControllerListener2.Extras obtainExtras(@Nullable Map<String, Object> datasourceExtras, @Nullable Map<String, Object> imageExtras, @Nullable Uri mainUri) {
        ...
        SettableDraweeHierarchy settableDraweeHierarchy = this.mSettableDraweeHierarchy;
        if (settableDraweeHierarchy instanceof GenericDraweeHierarchy) {
            strValueOf = String.valueOf(((GenericDraweeHierarchy) settableDraweeHierarchy).getActualImageScaleType());
            actualImageFocusPoint = ((GenericDraweeHierarchy) this.mSettableDraweeHierarchy).getActualImageFocusPoint();
        } else {
            strValueOf = null;
            actualImageFocusPoint = null;
        }
        return MiddlewareUtils.obtainExtras(COMPONENT_EXTRAS, SHORTCUT_EXTRAS, datasourceExtras, getDimensions(), strValueOf, actualImageFocusPoint, imageExtras, getCallerContext(), mainUri);
    }
