...
public class DebugControllerOverlayDrawable extends Drawable implements ImageLoadingTimeListener {
    ...
    private void prepareDebugTextParameters(Rect bounds, int numberOfLines, int maxLineLengthEm) {
        int iMin = Math.min(40, Math.max(10, Math.min(bounds.width() / maxLineLengthEm, bounds.height() / numberOfLines)));
        ...
        this.mStartTextYPx = this.mTextGravity == 80 ? bounds.bottom - 10 : bounds.top + 20;
    }
    ...
    private void addDebugText(Canvas canvas, String label, String value, int valueColor) {
        ...
        float fMeasureText2 = this.mPaint.measureText(value);
        this.mPaint.setColor(TEXT_BACKGROUND_COLOR);
        int i = this.mCurrentTextXPx;
        int i2 = this.mCurrentTextYPx;
        canvas.drawRect(i - 4, i2 + 8, i + fMeasureText + fMeasureText2 + 4.0f, i2 + this.mLineIncrementPx + 8, this.mPaint);
        this.mPaint.setColor(-1);
        canvas.drawText(str, this.mCurrentTextXPx, this.mCurrentTextYPx, this.mPaint);
        this.mPaint.setColor(valueColor);
        canvas.drawText(value, this.mCurrentTextXPx + fMeasureText, this.mCurrentTextYPx, this.mPaint);
    ...
    int determineSizeHintColor(int imageWidth, int imageHeight, @Nullable ScalingUtils.ScaleType scaleType) {
        ...
        if (iWidth > 0 && iHeight > 0 && imageWidth > 0 && imageHeight > 0) {
            if (scaleType != null) {
            ...
            float f6 = f4 * 0.5f;
            int iAbs = Math.abs(imageWidth - iWidth);
            int iAbs2 = Math.abs(imageHeight - iHeight);
            float f7 = iAbs;
            if (f7 < f2 && iAbs2 < f5) {
