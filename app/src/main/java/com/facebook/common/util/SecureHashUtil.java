...
public class SecureHashUtil {
    ...
    public static String makeSHA1HashBase64(byte[] bytes) throws NoSuchAlgorithmException {
        try {
    ...
    public static String makeMD5Hash(byte[] bytes) {
        return makeHash(bytes, Md5Utils.ALGORITHM);
    }

    public static String makeMD5Hash(InputStream stream) throws IOException {
        return makeHash(stream, Md5Utils.ALGORITHM);
    }
    ...
    public static String convertToHex(byte[] raw) throws UnsupportedEncodingException {
        ...
        for (byte b : raw) {
            ...
            sb.append((char) bArr[(b & 255) >>> 4]);
            sb.append((char) bArr[b & 15]);
        }
        return sb.toString();
    }

    private static String makeHash(byte[] bytes, String algorithm) throws NoSuchAlgorithmException {
        try {
    ...
    private static String makeHash(InputStream stream, String algorithm) throws NoSuchAlgorithmException, IOException {
        try {
            MessageDigest messageDigest = MessageDigest.getInstance(algorithm);
