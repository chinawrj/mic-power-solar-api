...
public final class ImageSizeUtils {
    ...
    public static ImageSize defineTargetSizeForView(ImageAware imageAware, ImageSize imageSize) {
        ...
        if (width <= 0) {
            iMin = imageSize.getWidth();
        } else {
            iMin = Math.min(width, imageSize.getWidth());
        }
        int height = imageAware.getHeight();
        if (height <= 0) {
            iMin2 = imageSize.getHeight();
        } else {
            iMin2 = Math.min(height, imageSize.getHeight());
        }
        return new ImageSize(iMin, iMin2);
    }
    ...
    public static int computeImageSampleSize(ImageSize imageSize, ImageSize imageSize2, ViewScaleType viewScaleType, boolean z) {
        ...
        int i = AnonymousClass1.$SwitchMap$com$nostra13$dcloudimageloader$core$assist$ViewScaleType[viewScaleType.ordinal()];
        if (i != 1) {
            if (i != 2) {
                iMax = 1;
            } else if (z) {
