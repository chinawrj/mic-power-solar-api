...
public class Layouts {
    ...
    private static void doSafeLayout(WXComponent wXComponent, float f, float f2) {
        try {
            ...
            int iNativeLayoutRenderObject = NativeRenderObjectUtils.nativeLayoutRenderObject(wXComponent.getRenderObjectPtr(), f, f2);
            WXEnvironment.isOpenDebugLog();
            if (iNativeLayoutRenderObject <= 0) {
                WXLogUtils.e(WXRecyclerTemplateList.TAG, " WXTemplateList doSafeLayout wrong template " + wXComponent.getAttrs().get(Constants.Name.Recycler.SLOT_TEMPLATE_CASE) + " cell height " + iNativeLayoutRenderObject);
            }
        } catch (Exception e) {
