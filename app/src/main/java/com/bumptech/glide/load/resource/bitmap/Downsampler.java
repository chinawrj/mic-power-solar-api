        ...
        if (shouldUsePool(type)) {
            if (i4 < 0 || i5 < 0 || !z2) {
                ...
                if (Log.isLoggable(TAG, 2)) {
                    Log.v(TAG, "Calculated target [" + iRound + Constants.Name.X + iRound2 + "] for source [" + i4 + Constants.Name.X + i5 + "], sampleSize: " + i9 + ", targetDensity: " + options.inTargetDensity + ", density: " + options.inDensity + ", density multiplier: " + f);
                }
            } else {
                iRound = i6;
                iRound2 = i7;
